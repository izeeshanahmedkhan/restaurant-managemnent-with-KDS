...
class ConversationController extends Controller
{
    ...
    public function getAdminMessage(Request $request): array
    {
        $limit = $request->has('limit') ? $request->limit : 10;
        $offset = $request->has('offset') ? $request->offset : 1;
        $messages = $this->conversation->where(['user_id' => auth('api')->user()->id])->latest()->paginate($limit, ['*'], 'page', $offset);
    ...
    public function storeAdminMessage(Request $request): JsonResponse
    {
        ...
        try {
            ...
            $data = [
                'title' => auth('api')->user()->f_name . ' ' . auth('api')->user()->l_name . translate(' send a message'),
                'description' => auth('api')->user()->id,
                'order_id' => '',
    ...
    public function getDeliverymanConversationList(Request $request): array
    {
        ...
        $adminLastConversation = $this->conversation->where(['user_id' => auth('api')->user()->id])
            ->latest('created_at')
            ->first();
