...
class POSController extends Controller
{
    ...
    public function index(Request $request): Renderable
    {
        ...
        $products = $this->product
            ->with(['shop_product_by_branch' => function ($q) {
                $q->where(['branch_id' => 1]);
            }])
        ...
        $tables = $this->table->where(['branch_id' => 1])->get();
//        dd($products);
        return view('shop.index', compact('categories', 'products', 'category', 'keyword', 'branch', 'tables', 'selectedTable', 'selectedCustomer'));
    }
    ...
    {
        $product = $this->product->with(['shop_product_by_branch' => function ($q) {
            $q->where(['branch_id' => 1]);
        }])->findOrFail($request->product_id);

        return response()->json([
            'success' => 1,
            'view' => view('shop._quick-view-data', compact('product'))->render(),
        ]);
    }
    ...
    public function variantPrice(Request $request): array
    {
        ...
        $branchProduct = $this->product_by_Branch->where(['product_id' => $request->id, 'branch_id' => 1])->first();
    ...
    public function addToCart(Request $request): JsonResponse
    {
        ...
        if (isset($branchProduct)) {
            ...
            if ($branchProduct->stock_type == 'daily' || $branchProduct->stock_type == 'fixed') {
                $available_stock = $branchProduct->stock - $branchProduct->sold_quantity;
    ...
    public function placeOrder(Request $request): \Illuminate\Http\JsonResponse
    {
        ...
        foreach ($cart as $c) {
            if (is_array($c)) {
                ...
                if ($product) {
                    ...
                    $branchProduct = $this->product_by_Branch->where(['product_id' => $c['id'], 'branch_id' => 1])->first();

                    //daily and fixed stock quantity validation
                    if ($branchProduct->stock_type == 'daily' || $branchProduct->stock_type == 'fixed') {
                        $available_stock = $branchProduct->stock - $branchProduct->sold_quantity;
                        if ($available_stock < $c['quantity']) {
                            Toastr::error(translate('stock limit exceeded'));
                            return back();
                        }
                    }
        ...
        if ($orderType == 'home_delivery') {
            $deliveryCharge = Helpers::get_delivery_charge(branchId: 1, distance: $distance, selectedDeliveryArea: $areaId, orderAmount: $totalPrice + $totalTaxAmount + $totalAddonTax);
        }
     ...
     * @return \Illuminate\Http\JsonResponse
     */
    public function placeCheckoutOrder(Request $request): \Illuminate\Http\JsonResponse
    {
        try {
            // Validate cart
            ...
            foreach ($cart as $item) {
                if (is_array($item)) {
                    ...
                    $orderDetails[] = [
                        ...
                        'tax_amount' => 0,
                        'discount_on_product' => $item['discount'] * $item['quantity'],
                        'discount_type' => 'discount_on_product',
            ...
            // Generate order ID
            $orderId = 100000 + \App\Model\Order::count() + 1;
            while (\App\Model\Order::find($orderId)) {
                $orderId++;
            }
            ...
            $order->order_type = $request->order_type === 'take_away' ? 'pos' : 'delivery';
            ...
            \App\Model\OrderDetail::insert($orderDetails);

            // Save payment details
            if ($request->payment_method === 'cash' || $request->payment_method === 'card') {
                $orderChangeAmount = new \App\Models\OrderChangeAmount();
                $orderChangeAmount->order_id = $order->id;
                $orderChangeAmount->order_amount = $order->order_amount;
                $orderChangeAmount->paid_amount = $request->paid_amount;
                $orderChangeAmount->save();
            ...
            if ($order->order_type === 'pos') {
                ...
                $notification->title = "New POS Order - Order #{$order->id}";
                $notification->description = "Order confirmed from POS system";
                $notification->status = 1;
                $notification->save();
            ...
            return response()->json([
                ...
                'order_id' => $order->id,
                'order' => $order,
                'receipt_url' => route('shop.receipt', $order->id)
            ]);
     ...
     * @return Renderable
     */
    public function showReceipt($id): Renderable
    {
        \Log::info('Receipt requested for order ID: ' . $id);
        
        $order = \App\Model\Order::with(['details.product', 'customer', 'branch'])->find($id);
        ...
        \Log::info('Order found: ' . $order->id . ', Status: ' . $order->order_status);
    ...
    public function checkoutPage()
    {
        ...
        // Get tables for branch 1, create sample tables if none exist
        $tables = $this->table->where(['branch_id' => 1, 'is_active' => 1])->get();
        
        if ($tables->isEmpty()) {
            // Create sample tables if none exist
            $sampleTables = [
                ['number' => 1, 'capacity' => 4, 'branch_id' => 1, 'is_active' => 1],
                ['number' => 2, 'capacity' => 6, 'branch_id' => 1, 'is_active' => 1],
                ['number' => 3, 'capacity' => 2, 'branch_id' => 1, 'is_active' => 1],
                ['number' => 4, 'capacity' => 8, 'branch_id' => 1, 'is_active' => 1],
                ['number' => 5, 'capacity' => 4, 'branch_id' => 1, 'is_active' => 1],
            ];
            
            foreach ($sampleTables as $tableData) {
                $this->table->create($tableData);
            }
            
            // Fetch the newly created tables
            $tables = $this->table->where(['branch_id' => 1, 'is_active' => 1])->get();
        }
        ...
        return view('shop.checkout', compact('cart', 'branch', 'selectedCustomer', 'selectedTable', 'tables', 'user'));
    }
    ...
    {
        return view('shop.partials.cart_items');
    }
    ...
    public function orderList(Request $request): Renderable
    {
        ...
        return view('shop.order.list', compact('orders', 'search', 'from', 'to'));
    }
    ...
    public function generateInvoice($id): JsonResponse
    {
        ...
        return response()->json([
            'success' => 1,
            'view' => view('shop.order.invoice', compact('order'))->render(),
        ]);
    }
