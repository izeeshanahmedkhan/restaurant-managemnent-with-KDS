<div class="card mt-4 view-details-container">
    <div class="card-body">
        <div class="bg-color rounded px-3 py-3">
            <div class="row align-items-center g-lg-3 g-2">
                <div class="col-xxl-3 col-lg-4">
                    <h5 class="mb-1 fz-16">{{translate('Setup Free Delivery Option')}}</h5>
                    <p class="mb-0 fz-12">{{ translate('Configure Free Delivery for Orders Above a Specific Value.') }}</p>
                </div>
                <div class="col-xxl-9 col-lg-8">
                    <div class="bg-white rounded p-20">
                        <div class="form-group m-0">

                            <div class="d-flex justify-content-between border rounded px-4 form-control">
                                <span class="pr-2">{{ translate('Status') }}</span>
                                <label class="switcher ml-auto mb-0">
                                    <input type="checkbox" class="switcher_input @if($branch?->delivery_charge_setup?->free_delivery_over_status == 1) off-free-delivery-over-{{$branch->id}} @else on-free-delivery-over-{{$branch->id}} @endif"
                                           name="free_delivery_over_status"
                                           {{ $branch?->delivery_charge_setup?->free_delivery_over_status == 1 ? 'checked' : '' }}>
                                    <span class="switcher_control"></span>
                                </label>
                            </div>
                        </div>
                        <div class="bg-color rounded p-20 mt-3 {{ $branch?->delivery_charge_setup?->free_delivery_over_status == 0 ? 'd-none' : '' }}" id="free-delivery-amount-section-{{$branch->id}}">
                            <form action="{{ route('admin.business-settings.restaurant.store-free-delivery-over-amount') }}" method="POST">
                                @csrf
                                <input type="hidden" name="branch_id" id="" value="{{ $branch->id }}">
                                <input type="hidden" name="free_delivery_over_status" id="" value="1">
                                <div class="form-group">
                                    <label for="free_delivery_over_amount">{{ translate('Free Delivery Over') }} ({{ Helpers::currency_symbol() }})</label>
                                    <input type="number" class="form-control" name="free_delivery_over_amount" min="0" max="9999999999" step="0.0001"
                                        value="{{ $branch?->delivery_charge_setup?->free_delivery_over_amount }}" id="free_delivery_over_amount" placeholder="Ex: 10" required>
                                </div>
...
<!-- Confirmation Modal for Turning On a New Setup -->
<div class="modal fade" id="off-free-delivery-confirm-modal-{{$branch->id}}" tabindex="-1" role="dialog" aria-labelledby="offFreeDeliveryConfirmModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
            ...
            <form action="{{ route('admin.business-settings.restaurant.store-free-delivery-over-status') }}" method="POST">
               @csrf
                <input type="hidden" name="branch_id" id="" value="{{ $branch->id }}">
                <input type="hidden" name="free_delivery_over_status" id="" value="0">
                <div class="d-flex justify-content-center mb-4 gap-3">
                    <button type="button" class="btn btn-secondary" id="free-delivery-cancel-btn-{{$branch->id}}" data-dismiss="modal">{{ translate('Cancel') }}</button>
                    <button type="submit" class="btn btn-primary" id="confirmChange-{{$branch->id}}">{{ translate('Yes') }}, {{ translate('Change') }}</button>
                </div>
            </form>
...
@push('script_2')
    ...
    <script>
        $(document).ready(function() {
            $('.off-free-delivery-over-{{$branch->id}}').on('click', function (e){
                e.preventDefault();

                $('#off-free-delivery-confirm-modal-{{$branch->id}}').modal('show');
            });

            $('.on-free-delivery-over-{{$branch->id}}').on('click', function (){
                $('#free-delivery-amount-section-{{$branch->id}}').removeClass('d-none');
            });
        });
