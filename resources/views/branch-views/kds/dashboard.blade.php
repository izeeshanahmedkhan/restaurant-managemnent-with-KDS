                    ...
                    <div class="row align-items-center py-4">
                        ...
                        <div class="col-md-6">
                            <div class="d-flex justify-content-end align-items-center">
                                ...
                                @if($selectedBranch)
                                    <div class="kds-branch-info me-3">
                                        <div class="kds-branch-label">Branch:</div>
                                        <div class="kds-branch-name">{{ $selectedBranch->name }}</div>
                                    </div>
                                    <input type="hidden" id="branch-selector" value="{{ $selectedBranch->id }}">
                                @else
                                    <div class="kds-branch-info me-3">
                                        <div class="kds-branch-label text-danger">No Branch Assigned</div>
                                    </div>
                                @endif
                                <div class="kds-user-info me-3">
                                    <div class="kds-user-name">Branch Manager</div>
                                    <div class="kds-user-role">Branch Staff</div>
                                </div>
                            </div>
                        </div>
                    </div>
...
$(document).ready(function() {
    ...
    // Initialize KDS - Branch users automatically get their assigned branch
    if (currentBranch) {
        loadOrders(currentBranch);
        loadItemsSummary(currentBranch);
    } else {
        console.warn('No branch assigned to this user');
        clearOrders();
    }
    ...
    function loadItemsSummary(branchId) {
        ...
        $.ajax({
            url: '{{ route("branch.kds.items-summary") }}',
            method: 'GET',
            data: { branch_id: branchId },
            headers: {
                'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content'),
    ...
    function displayItemsSummary(items) {
        if (items.length === 0) {
            $('#items-summary').html(`
                <div class="text-center py-4 text-muted">
                    <i class="tio-list text-2xl mb-2"></i>
                    <p>No items</p>
                </div>
            `);
            return;
    ...
    function updateOrderStatus(orderId, newStatus) {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                $.ajax({
                    url: '{{ route("branch.kds.update-status") }}',
                    method: 'POST',
                    data: {
                        order_id: orderId,
                        status: newStatus,
