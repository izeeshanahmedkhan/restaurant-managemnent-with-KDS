...
<head>
    ...
    <title>Kitchen Display System - Branch Dashboard</title>
...
<body class="kds-body">
    <div class="kds-container">
        ...
        <header class="kds-header">
            ...
            <div class="kds-header__controls">
                ...
                <a href="{{ route('branch.dashboard') }}" class="kds-header__btn kds-header__btn--primary">
    ...
    <script>
        ...
        // Override API endpoints for branch
        window.KDS_CONFIG = {
            apiEndpoints: {
        ...
        $(document).ready(function() {
            console.log('Branch KDS: Page loaded, initializing...');
            
            // Initialize with assigned branch - Branch users automatically get their assigned branch
            const branchId = $('#branch-selector').val();
            if (branchId) {
                console.log('Branch KDS: Using assigned branch:', branchId);
                // Initialize KDS with the assigned branch
                if (window.KDS && window.KDS.init) {
                    window.KDS.init();
                }
            } else {
                console.warn('Branch KDS: No branch assigned to this user');
            }
            
            // Override the fetchUpdates function to use branch endpoints
            const originalFetchUpdates = window.KDS.fetchUpdates;
            window.KDS.fetchUpdates = function() {
                const branchId = $('#branch-selector').val();
                if (!branchId) {
                    console.warn('Branch KDS: No branch selected');
                    return;
                }
                ...
                .done(function(response) {
                    if (!response || !response.orders) {
                        console.warn('Branch KDS: Invalid response format');
                        return;
                    }
                ...
                .fail(function(xhr, status, error) {
                    console.error('Branch KDS: Failed to fetch updates:', status, error);
            ...
            window.KDS.updateOrderStatus = function(orderId, newStatus) {
                ...
                .done(function(response) {
                    if (response.ok) {
                        console.log('Branch KDS: Order', orderId, 'status updated to', newStatus);
                        // Refresh data
                ...
                .fail(function(xhr, status, error) {
                    console.error('Branch KDS: Failed to update order status:', status, error);
