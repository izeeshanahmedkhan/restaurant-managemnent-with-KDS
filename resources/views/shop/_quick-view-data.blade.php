...
<form id="add-to-cart-form">
    ...
    <div class="modal-body pt-0">
        <div class="d-flex flex-wrap gap-3">
            ...
            <?php
                $pb = $product->shop_product_by_branch->toArray();
            ...
            <div class="details">
                ...
                <div>
                    ...
                    @if($halalTagStatus && $pb[0]['halal_status'] )
                        <span class="badge badge-soft-secondary fz-12 px-3 py-2 text-title">
                        <img width="13" class="mr-1" src="{{asset('assets/admin/img/halal-tags.png')}}" alt="{{ translate('halal tag') }}">
                        {{ translate("Halal") }}
                    </span>
                    @endif
                </div>
            ...
            <div class="col-12">
                ...
                @endif
                    ...
                    <div class="d-flex flex-column gap-3">
                        ...
                        @if (isset($product->shop_product_by_branch) && count($product->shop_product_by_branch))
                            @foreach($product->shop_product_by_branch as $branch_product)
...
<script>
    ...
    // Override quick-view add to cart to use SHOP routes
    $('.add-to-cart-button').off('click').on('click', function () {
        $.post(window.SHOP_ROUTES.add, $('#add-to-cart-form').serialize(), function (response) {
            if (response && response.data) {
                // close modal
                $('#quickViewModal').modal('hide');
                // refresh cart sidebar
                $.get(window.SHOP_ROUTES.cart + '/items', function (data) {
                    $('#cart-container').html(data);
                    // update badge if present
                    var itemCount = $(data).find('.cart-item').length;
